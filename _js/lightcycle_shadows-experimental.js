...
$(function () {
	var LightCycle = new function () {
		...
		function setupListeners() {
			$('#actions')
				.bind('playerScored', function(e) {
				  	score += level*10;
					updateScore();
				})
				.bind('botDied', function(e) {
					playBoom();
				  	giveBonus(BONUS);
				})
				.bind('endRound', function (e) {
					playBoom();
					endRound(e.isBot);
				});
		...
		function setupSound() {
			...
			soundManager.onready(function() {
				...
				//soundManager.createSound({ id: '2', url: 'LS/2.mp3' });
				//soundManager.createSound({ id: '3', url: 'LS/3.mp3' });
				//soundManager.createSound({ id: '4', url: 'LS/4.mp3' });		
			});
		}
		...
		function playLoop() {
			if (!loopIsPlaying) {
				loopIsPlaying = true;
				//soundManager.play(((Math.floor(Math.random()*4)+1).toString()),{
		...
		function endRound(isBot){
			...
			$actions.find('a.enter').click(function (e) {
				e.preventDefault();
				(function checkIfAnimationIsFinished(i){
		...
		function giveBonus(amount) {
			score += amount;
			$theBonus.show().text(amount+' BONUS!').fadeOut(2000);
			updateScore();
		}
		...
		function startGame() {
			running = true;
			if ( level === 1) score = 0;
			$('#actions').find('a').unbind().end().html('');
			$('#theLevel').html(level);
			updateScore();
			metersTravelled = 0;
				...
				for (var i=0, player; i < _players.length; i++) {
					...
					if ( player.p === null || (player.p.state !== 0 && !_firstRun) ) {
						if (player.isBot && _players.length > 2) {
							$actions.trigger({type:'botDied'});
							renderer.playerSelfdestruct(player.index);
							_players.splice(i,1);
							continue;
						};
						renderer.playerSelfdestruct(player.index);
						$actions.trigger({type:'endRound', isBot: player.isBot});
						return;
					};
		...
		function ThreeRenderer() {
			...
			var container, camera, scene, renderer, composer, basicPlane, gameboard, players, cameraPosition, cameraMode, grid, starfield;
		
			container = document.getElementById( 'maze' );
			renderer = new THREE.WebGLRenderer( { clearColor: 0x000000, clearAlpha: 1, antialias: true } );
			renderer.shadowMapEnabled = true;
			renderer.setSize( window.innerWidth-10, window.innerHeight-10-$('header').height() );
			...
			var test = new THREE.ShaderPass( THREE.ShaderExtras[ "vignette" ] );
			//test.uniforms[ 'scale' ].value = 100;
			composer.addPass( renderModel );
			composer.addPass( effectFXAA );
			composer.addPass( effectBloom );
			//composer.addPass( test )
			...
			//grid = new THREE.Mesh( new THREE.PlaneGeometry( width, height, 1, 1), new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( 'GRID.png' ) } ) );
			grid.position.set( -.5, .5, -.5 );
			scene.add( grid );
			
			// starfield
			/*
			starfield = new THREE.Mesh( new THREE.SphereGeometry( width*2, 10, 10), new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( '1024x512_2.png' ) } ) );
			starfield.rotation.x = Math.PI/2;
			starfield.doubleSided = true;
			scene.add( starfield );
			*/
			...
			this.materials = (function () {
				for (var i=0, m = []; i < levels.length; i++) {
					m.push(new THREE.MeshBasicMaterial( { color: levels[i].clr, shading: THREE.FlatShading }));
				};
				return m;
			})();
			...
			function turnPlayer(stackItem, currentPlayer) {
				// increase stooopidity over time
				//stackItem.player.turnLikeliness = (stackItem.player.turnLikeliness>5) ? stackItem.player.turnLikeliness-1 : 5;
				//stackItem.player.irrationalityFactor = (stackItem.player.irrationalityFactor>5) ? stackItem.player.irrationalityFactor-1 : 5;

				currentPlayer.currentMesh = new THREE.Mesh( basicPlane, stackItem.player.material );
				currentPlayer.currentMesh.doubleSided = true;
				currentPlayer.currentMesh.castShadow = true;
				currentPlayer.currentMesh.receiveShadow = true;
				...
				switch (stackItem.p.state){
					case 1: currentPlayer.rotation.z = (Math.PI/180)*270; 	break;
					case 2: currentPlayer.rotation.z = (Math.PI/180)*180;	break;
					case 3: currentPlayer.rotation.z = (Math.PI/180)*90;	break;
			...
			function setupLights() {
				...
				light.position.set( 1000, 0, 100 ); // r
				...
				light.position.set( -1000, 0, 100 ); // l
				light.castShadow = true;
				scene.add( light );

				light = new THREE.SpotLight( 0x666666 );
				light.position.set( 0, 1000, 100 ); // u
				light.castShadow = true;
				scene.add( light );

				light = new THREE.SpotLight( 0xffffff );
				light.position.set( 0, -1000, 100 ); // b
				light.castShadow = true;
				scene.add( light );
			}
